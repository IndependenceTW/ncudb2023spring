	...
	BEGIN
		...
		IF(@trend = 1 AND @today_bias > 0 AND @today_bias < @bias_threshold)
			BEGIN
                ...
                SELECT @back_count=COUNT(*) 
                FROM @temp_table t1
                WHERE EXISTS(
                    SELECT [date] 
                    FROM dbo.find_date_func(@date, @backward_day, 0, 0)
                    WHERE [date] = t1.date
                )
                AND ((t1.today_c - @today_c) / @today_c) * 100  > @backward_threshold
				
                If @back_count > 0
                BEGIN
				/* 看後forward_day是否有回升，收盤價差大於forward_threshold，且都沒有低過均線 */
                    DECLARE @forward_count INT
                    SELECT @forward_count=COUNT(*)
                    FROM @temp_table t1
                    WHERE EXISTS(
                        SELECT [date]
                        FROM dbo.find_date_func(@date, @forward_day, 0, 1)
                        WHERE [date] = t1.date
                    )
                    AND t1.today_c > t1.today_MA
				/*確認該日期是否已經存在於表中*/
                    SELECT TOP (1) @return_date = t1.date
                    FROM @temp_table t1
                    WHERE EXISTS(
                        SELECT [date]
                        FROM dbo.find_date_func(@date, @forward_day, 0, 1)
                    ...
                    AND t1.today_c > t1.today_MA
                    AND ((t1.today_c - @today_c) / @today_c) * 100  > @forward_threshold
                    AND NOT EXISTS(
                        SELECT [date]
                        FROM @rule_3
                        WHERE [date] = t1.date
                    ...
                    IF @forward_count = @forward_day AND @return_date IS NOT NULL
                    BEGIN
                        INSERT INTO @rule_3 (date,buy_or_sell)
                        VALUES (@return_date, 1)
                    END
                END
		...
		ELSE IF(@trend = -1 AND @today_bias < 0 AND abs(@today_bias) < @bias_threshold)
			BEGIN
                ...
                DECLARE @back_count1 INT
                SELECT @back_count1=COUNT(*)
                FROM @temp_table t1
                WHERE EXISTS(
                    SELECT [date]
                    FROM dbo.find_date_func(@date, @backward_day, 0, 0)
                ...
                AND ((@today_c - t1.today_c) / t1.today_c) * 100  > @backward_threshold

                IF @back_count1 > 0
                BEGIN
				/* 看後forward_day天是否有回降，收盤價差的絕對值大於forward_threshold，且都沒有高過均線 */
                    DECLARE @forward_count1 INT
                    SELECT @forward_count1=COUNT(*)
                    FROM @temp_table t1
                    WHERE EXISTS(
                        SELECT [date]
                        FROM dbo.find_date_func(@date, @forward_day, 0, 1)
                    ...
                    AND t1.today_c < t1.today_MA
                    AND ((@today_c - t1.today_c) / t1.today_c) * 100  > @forward_threshold
                    AND NOT EXISTS(
                        SELECT [date]
                        FROM @rule_3
                        WHERE [date] = t1.date
                    ...
                    IF @forward_count1 = @forward_day AND @return_date IS NOT NULL
                    BEGIN
                        INSERT INTO @rule_3 (date,buy_or_sell)
                        VALUES (@return_date, -1)
                    END
