USE [ncudb]
GO
/****** Object:  UserDefinedFUNCTION [dbo].[candlestick_type]    Script Date: 2023/4/7 下午 12:58:53 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE FUNCTION [dbo].[candlestick_type](
	@company varchar(10),
	@date varchar(10)
    )
...
BEGIN
	...
	SELECT @today_c=c, @today_o=o FROM stock_price WHERE stock_code=@company and [date] = @date;
	SELECT @yesterday_c=c FROM stock_price WHERE stock_code=@company and [date] = @last_working_date;
	...
	ELSE IF @stick < @stick_short SET @type=1;
